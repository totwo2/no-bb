...
**Want to try it now?** [Quick start](#6-quick-start) to launch the middleware and integrate in one command; after installing, run the [30-second health check](#7-post-install-health-check-confirm-youre-not-silently-bypassed) to confirm the constraint is really active.
...
## 7. Post-install health check: confirm you're not silently bypassed

> "Bypassed" = you believe the constraint is working but it isn't (the platform dropped the injection, or traffic never went through the pipe) — and nothing tells you. Run one check after installing:

```bash
python3 scripts/wrap.py check
```

Layered self-test: **L0** middleware process / **L1** external models actually pointing at the middleware / **L2** converge hook mounted / **L3** hook dry-run produces valid injection / **L4** real pipe traffic in the last 30 min. Each failing layer prints a fix command; config files overwritten/migrated by the platform are detected with guidance.

The check ends with a "30-second dual verification" — two answers in one go: **is the constraint really active? and how much does it actually save?**

- **Pipe scenario** (external BYO-key models): `bench/verify_savings.py` runs the A/B and prints savings directly (median reasoning chars and wall-clock; reference: 1914→145 chars / 13.1s→1.4s).
- **Built-in model scenario** (WorkBuddy built-in free models via hook): two-message A/B — ① send a test question; the assistant confirms whether the constraint reached its input (it can see it) and records the reply length as baseline; ② the assistant pauses the constraint and you resend the same question. Compare — the saving is shown with real numbers (chars, elapsed time).

## 8. Verify the savings (quantify it after installing)
...
> Note: the three examples in Section 1, the finance single-problem A/B (-83%/-82%), and the 9,815-block statistic are real API calls from **exploratory experiments during development** — the evaluation pipeline did not yet exist then, and the raw data was not persisted, so they fall outside the reproducible scope of this section; every formal evaluation number inside `results/` is fully reproducible.
